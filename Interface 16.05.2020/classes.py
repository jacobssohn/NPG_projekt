...
import pygame
import random
from word_base import word_base as wb
...
class Game:
    ...
    def __init__(self,win):
        ...
        self.english_button = button((225, 0, 0), 360, 460, 159, 72)
        self.polish_button = button((255, 0, 0), 620, 460, 158, 72)
        ...
        self.bg_ch = pygame.image.load('backgrounds/bg_ch.jpg')
        ...
        self.polish0, self.back0, self.easy0, self.medium0, self.hard0, self.learning0 = self.pol_buttons0

        self.start1, self.challange1, self.settings1, self.english1,\
        self.polish1, self.back1, self.easy1, self.medium1, self.hard1, self.learning1 = self.pol_buttons1
    ...
    def change_language(self,language: str) ->None: #funkcja odpowiedzialna za zmiane języka
        if (language == "polish"):
            ...
            self.start0, self.challange0, self.settings0, self.english0, \
            self.polish0, self.back0, self.easy0, self.medium0, self.hard0, self.learning0 = self.ang_buttons0
            self.start1, self.challange1, self.settings1, self.english1, \
            self.polish1, self.back1, self.easy1, self.medium1, self.hard1, self.learning1 = self.ang_buttons1
    ...
    def redrawWindow(self,mode): #funkcja odpowiedzialna za narysowanie okna zależnie od aktualnego trybu
        ...
        if mode == "menu":
            ...
            if (self.language == "polish"):
                self.win.blit(pygame.image.load(self.polish1), (620, 460))
                self.win.blit(pygame.image.load(self.english0), (360, 460))
                if(self.english_button.isOver(pos)):
                    self.win.blit(pygame.image.load(self.english1), (360, 460))
            elif(self.language == "english"):
                self.win.blit(pygame.image.load(self.english1), (360, 460))
                self.win.blit(pygame.image.load(self.polish0), (620, 460))
 ...
 #część z mechaniki
                if (event.type == pygame.KEYDOWN and  # wyjątki które nie powodują wpisania nazwy klawisza
                        ...
                        ) and (self.mode == "easy" or self.mode == "medium" or self.mode == "hard")
                ):
                    pass
                    ...
                    self.input_text += letter
                      ...
                      (self.mode == "easy" or self.mode == "medium" or self.mode == "hard")):  # usuwanie liter
                    self.input_text = self.input_text[: len(self.input_text) - 1]

                elif (event.type == pygame.KEYDOWN and (self.mode == "easy" or self.mode == "medium" or self.mode == "hard")):  # wpisywanie liter
